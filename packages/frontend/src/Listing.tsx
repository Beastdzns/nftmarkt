...
function Listing() {
  ...
  useEffect(() => {
    ...
    suiClient
      ...
      .then((objects) => {
        const balanceItems = objects.data
          .map((o) => o.data?.content as any)
          .filter((content) => content.fields.balance)
          .map((content) => ({
        ...
        const mockNfts: Item[] = [
          {
            id: 'mock-nft-1',
            name: 'Mock NFT #1',
            imgUrl: 'https://via.placeholder.com/150/FF5733/FFFFFF?text=NFT+1',
            type: 'mock::type::nft',
          },
          {
            id: 'mock-nft-2',
            name: 'Mock NFT #2',
            imgUrl: 'https://via.placeholder.com/150/33FF57/FFFFFF?text=NFT+2',
            type: 'mock::type::nft',
          },
          {
            id: 'mock-nft-3',
            name: 'Mock NFT #3',
            imgUrl: 'https://via.placeholder.com/150/5733FF/FFFFFF?text=NFT+3',
            type: 'mock::type::nft',
          },
        ];

        // Combine real NFTs, balances, and mock NFTs
        setNftItems([...balanceItems, ...nftItems, ...mockNfts]);
        setLoading(false);
      });
  }, [account]);
...
function NftItem({ item }: { item: Item }) {
  ...
  return (
    <>
      <div className="w-full md:w-1/3 xl:w-1/4 p-6 flex flex-col">
        ...
        <div className="flex justify-between pt-3">
          <div className="">
            ...
            {listed && <p className="pt-1 text-gray-900">{item.price}</p>}
          </div>
          <button
      ...
      <dialog
        ref={listingModal}
        className="relative w-80 rounded-lg shadow max-w-screen-md"
      >
        <div className="flex items-center justify-between p-4 md:p-5 border-b rounded-t">
          <h3 className="text-xl font-semibold text-gray-900">Listing</h3>
          <button
            type="button"
                ...
                onClick={() => {
                  ...
                  txb.moveCall({
                    ...
                    arguments: [
                      txb.object(VAULT_ID),
                      txb.pure(
                        bcs.vector(bcs.U8).serialize(fromHexString(invoice.rHash))
                      ),
                      txb.object(item.id),
                      txb.pure(
                        bcs
                          .vector(bcs.U8)
                          .serialize(
                            new TextEncoder().encode(invoice.paymentRequest)
                          )
                      ),
                    ],
                  });
